...
impl BlockingResumableUploader {
        ...
        R: Read,
    {
        ...
        let checkpoint = ResumableUploadCheckpoint::new(upload_id, self.options.part_size);
        ...
        loop {
            ...
            let uploaded = loop {
                match backend.upload_part(&checkpoint.upload_id, part_number, &chunk) {
                    Ok(part) => break normalize_part(part, part_number, chunk.len()),
                    Err(source) => {
    ...
    fn blocking_retry_failure_returns_checkpoint_for_resume() {
        ...
        match error {
            ResumableUploadError::PartUploadFailed {
                ...
                assert!(checkpoint.completed_parts.contains_key(&1));
